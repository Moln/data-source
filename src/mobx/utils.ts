import {
    entries,
    IArrayDidChange,
    IMapDidChange, intercept,
    IObjectDidChange, IObjectWillChange,
    isObservableArray,
    isObservableMap,
    isObservableObject, IValueWillChange, observe, values
} from "mobx";
import {IArrayWillChange, IArrayWillSplice, IInterceptor} from "mobx/src/internal";
...
export function deepObserve<T = any>(
    ...
    const processChange = processChangeCall(observeRecursively, unobserveRecursively, entrySet)
...
type IInterceptHandler<T> = IInterceptor<IObjectWillChange<T> | IArrayWillChange<T> | IArrayWillSplice<T> | IValueWillChange<T>>;
type IInterceptChange<T> = Parameters<IInterceptHandler<T>>[0];


export function deepIntercept<T = any>(
    target: T,
    listener: (change: IInterceptChange<T>, path: string[], root: T) => IInterceptChange<T> | null
): IDisposer {

    const entrySet = new WeakMap<any, Entry>()

    const processChange = processChangeCall(interceptRecursively, uninterceptRecursively, entrySet)
    ...
    function interceptRecursively(thing: any, parent: Entry | undefined, path: string) {
        if (isRecursivelyObservable(thing)) {
            const entry = entrySet.get(thing)
            if (entry) {
                if (entry.parent !== parent || entry.path !== path)
                ...
                entrySet.set(thing, entry)
                entries(thing).forEach(([key, value]) => interceptRecursively(value, entry, key))
            }
        }
    }
