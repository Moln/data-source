...
const Comparer = {
  ...
  create<T>(sort: SortOptions1 | SortOptions2<T>): CompareFn {
    const compare =
      (sort as SortOptions1).compare ||
      this.compare((sort as SortOptions2<T>).field);
...
function isFilters<T>(
  f: DataSourceFilterItem<T> | DataSourceFilters<T>
): f is DataSourceFilters<T> {
  return 'logic' in f;
}

function compareDescriptor<T>(
  f1: DataSourceFilterItem<T> | DataSourceFilters<T>,
  f2: DataSourceFilterItem<T> | DataSourceFilters<T>
): boolean {
...
function fieldComparer<T>(
  a: DataSourceFilterItem<T> | DataSourceFilters<T>,
  b: DataSourceFilterItem<T> | DataSourceFilters<T>
) {
