...
import {action, computed, intercept, makeObservable, observable, observe, runInAction} from "mobx";
import {createModel, DEFAULT_SCHEMA, Model, normalizeFilter, normalizeSort, Schema} from "./";
...
export class DataSource<T extends object = object> implements DataSourceInterface<T> {
    ...
    addFilter(filter: DataSourceFilterItem<T>) {
        if (! this.filter) {
            ...
            const item = this.filter.filters.find((item) =>
                'field' in item &&
    ...
    removeFilter(field: string, operator?: OperatorKeys) {
        ...
        const idx = this.filter.filters.findIndex((item) =>
            'field' in item &&
            item.field === field &&
