...
export function expr(expression: string, safe?: string | boolean, paramName?: string): string {
...
export function getter<T>(expression: keyof T | string, paramName?: string): (data: T) => any {
    const key = expression + (paramName || '');
    return (
        getterCache[key] = getterCache[key]
        || new Function("d", "return " + expr(expression as string, paramName))
    ) as (data: T) => any;
}

export function setter<T>(expression: keyof T | string): (data: T, value: any) => void {
    return (
        setterCache[expression as string] = setterCache[expression as string]
            || new Function("d,value", expr(expression as string) + "=value")
    ) as (data: T, value: any) => void;
}
...
export function toFilter<T extends object>(params: {[key in keyof T]: any}): DataSourceFilters<T> {
