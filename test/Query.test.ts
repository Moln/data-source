...
describe('Query', () => {
    ...
    it('should filter ok', function () {
        ...
        const q7 = q.filter({logic: "or", filters: [{field: "id", value: 2}, {field: "id", value: 4}]});
    ...
    it('should group ok', function () {
        ...
        expect(((q3.toArray()[0].items[0] as GroupItem<typeof data[0]>).items as typeof data)[0].id).toBeLessThan(11);
    });
    ...
    it('should aggregate', function () {
        const data = genData();
        const q = new Query(data);
        const rs = q.aggregate([{field: "name", aggregate: "count"}, {field: "age", aggregate: "max"}]);
